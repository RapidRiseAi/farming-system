...
const ASSET_STATUS_TRANSITIONS: Record<
  (typeof FARM_ASSET_STATUSES)[number],
  Set<(typeof FARM_ASSET_STATUSES)[number]>
> = {
...
const FARM_WORK_REQUEST_STATUSES = [
  'new',
  'triaged',
  'waiting_approval',
  'approved',
...
const FARM_WORK_ORDER_STATUSES = [
  'open',
  'triaged',
  'approved',
  'in_progress',
...
const FARM_WORK_REQUEST_STATUS_SET = new Set<string>(
  FARM_WORK_REQUEST_STATUSES
);
const FARM_WORK_ORDER_STATUS_SET = new Set<string>(FARM_WORK_ORDER_STATUSES);
const FARM_STOCK_REQUEST_STATUSES = [
  'new',
  'pending_approval',
  'approved',
  'partially_issued',
...
const FARM_STOCK_REQUEST_STATUS_SET = new Set<string>(
  FARM_STOCK_REQUEST_STATUSES
);
const STOCK_REQUEST_TRANSITIONS: Record<
  (typeof FARM_STOCK_REQUEST_STATUSES)[number],
  Set<(typeof FARM_STOCK_REQUEST_STATUSES)[number]>
> = {
...
const REQUEST_TRANSITIONS: Record<
  (typeof FARM_WORK_REQUEST_STATUSES)[number],
  Set<(typeof FARM_WORK_REQUEST_STATUSES)[number]>
> = {
...
const WORK_ORDER_TRANSITIONS: Record<
  (typeof FARM_WORK_ORDER_STATUSES)[number],
  Set<(typeof FARM_WORK_ORDER_STATUSES)[number]>
> = {
...
const CROP_ACTIVITY_TRANSITIONS: Record<
  (typeof CROP_ACTIVITY_STATUSES)[number],
  Set<(typeof CROP_ACTIVITY_STATUSES)[number]>
> = {
...
async function getFarmContext() {
  ...
  const { data: profile } = await supabase
    ...
    .select(
      `id,role,full_name,farm_account_id:${FARM_ACCOUNT_COLUMN},${FARM_ACCOUNT_COLUMN}`
    )
    .eq('id', user.id)
    .maybeSingle();
  ...
  if (!farmAccountId || !FARM_STAFF_ROLES.has(profile.role)) {
    return null;
  }
...
function toPointLiteral(
  latitude: FormDataEntryValue | null,
  longitude: FormDataEntryValue | null
): string | null {
...
async function addHistory(
  ...
  await ctx.supabase.from('farm_entity_history').insert({
    workshop_account_id: ctx.profile.farmAccountId,
...
function computeNextServiceDueAt(
  baseDate: Date,
  intervalType: string,
  ...
  if (intervalType === 'days') {
    ...
    dueDate.setUTCDate(
      dueDate.getUTCDate() + Math.max(1, Math.round(intervalValue))
    );
...
export async function completeFarmOnboarding(
  formData: FormData
): Promise<void> {
  ...
  if (farmName) {
    await ctx.supabase
      .from(FARM_ACCOUNTS_TABLE)
      .update({ name: farmName })
      .eq('id', ctx.profile.farmAccountId);
  }
  ...
  if (sampleData) {
    ...
    const { data: sampleTask } = await ctx.supabase
      ...
      .insert({
        ...
        description:
          'Walk every active area and confirm PPE, signage, and emergency contacts are available.',
    ...
    if (sampleTask?.id) {
      await addHistory(ctx, 'farm_task', sampleTask.id, 'created', {
        seeded: true
      });
    }
  }

  await ctx.supabase.from('farm_onboarding_progress').upsert(
    {
      workshop_account_id: ctx.profile.farmAccountId,
      completed: true,
...
export async function createFarmAsset(formData: FormData): Promise<void> {
  ...
  const serviceIntervalType = String(
    formData.get('serviceIntervalType') ?? 'hours'
  ).trim();
  const serviceIntervalValue =
    toNullableNumber(formData.get('serviceIntervalValue')) ?? 250;
  const lastServiceMeter =
    toNullableNumber(formData.get('lastServiceMeter')) ?? 0;
  if (!name) return;
  const status = String(formData.get('status') ?? 'active').trim();
  if (!FARM_ASSET_STATUS_SET.has(status)) return;
  const now = new Date();
  const nextServiceDueAt = computeNextServiceDueAt(
    now,
    serviceIntervalType,
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      name,
      asset_code:
        String(formData.get('assetCode') ?? '').trim() ||
        `ASSET-${now.getTime()}`,
      qr_token:
        String(formData.get('qrToken') ?? '').trim() || crypto.randomUUID(),
      ...
      current_hours:
        Number(String(formData.get('currentHours') ?? '').trim() || '0') ||
        null,
      current_odometer_km:
        Number(String(formData.get('currentOdometerKm') ?? '').trim() || '0') ||
        null,
  ...
  if (!data?.id) return;
  await addHistory(ctx, 'farm_asset', data.id, 'created', {
    name,
    asset_type: assetType
  });
...
export async function updateFarmAsset(formData: FormData): Promise<void> {
  ...
  const { data: existingAsset } = await ctx.supabase
    ...
    .select('status')
    .eq('id', assetId)
    .eq(FARM_ACCOUNT_COLUMN, ctx.profile.farmAccountId)
    .maybeSingle();
  if (!existingAsset) return;

  const currentStatus = String(
    existingAsset.status ?? ''
  ).trim() as (typeof FARM_ASSET_STATUSES)[number];
  const allowedTransitions = ASSET_STATUS_TRANSITIONS[currentStatus];
  if (
    currentStatus !== status &&
    !allowedTransitions?.has(status as (typeof FARM_ASSET_STATUSES)[number])
  ) {
    return;
  }
  ...
  const serviceIntervalValue = toNullableNumber(
    formData.get('serviceIntervalValue')
  );
  ...
  await ctx.supabase
    ...
    .update({
      ...
      next_service_due_at: nextServiceDueAt,
      service_due_override_reason: toNullable(
        formData.get('serviceDueOverrideReason')
      ),
      criticality: String(formData.get('criticality') ?? 'medium'),
      notes: toNullable(formData.get('notes')),
      current_hours:
  ...
  await addHistory(
    ...
    'farm_asset',
    assetId,
    status === 'retired' ? 'archived' : 'updated',
    { status }
  );
  revalidatePath('/farm/assets');
}
...
export async function fastUpdateAssetMeter(formData: FormData): Promise<void> {
  ...
  const meterPatch =
    meterType === 'distance_km'
      ? { current_odometer_km: Math.round(meterReading) }
      : { current_hours: meterReading };
  await ctx.supabase
    .from('farm_assets')
    .update(meterPatch)
    .eq('id', assetId)
  ...
  await addHistory(ctx, 'farm_asset', assetId, 'meter_updated', {
    meter_type: meterType,
    meter_reading: meterReading
  });
  revalidatePath('/farm/assets');
...
export async function overrideAssetServiceDue(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  await addHistory(ctx, 'farm_asset', assetId, 'service_due_overridden', {
    next_service_due_at: dueAt,
    reason
  });
  revalidatePath('/farm/assets');
...
export async function openAssetDowntime(formData: FormData): Promise<void> {
  ...
  await addHistory(ctx, 'farm_asset', assetId, 'downtime_started', {
    fault_id: fault?.id,
...
export async function closeAssetDowntime(formData: FormData): Promise<void> {
  ...
  await addHistory(ctx, 'farm_asset', assetId, 'downtime_closed', {
    fault_id: faultId,
    closure_summary: closureSummary
  });
  revalidatePath('/farm/assets');
...
export async function linkAssetDocument(formData: FormData): Promise<void> {
  ...
  await addHistory(ctx, 'farm_asset', assetId, 'document_linked', {
    document_id: data?.id,
    title
  });
  revalidatePath('/farm/assets');
...
export async function createFarmSite(formData: FormData): Promise<void> {
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      centroid: toPointLiteral(
        formData.get('centroidLat'),
        formData.get('centroidLng')
      ),
...
export async function createFarmProductionUnit(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx || !isManager(ctx.profile.role)) return;
...
export async function updateFarmSite(formData: FormData): Promise<void> {
  ...
  await addHistory(ctx, 'farm_site', siteId, 'updated', {
    status: String(formData.get('status') ?? 'active')
  });
  revalidatePath('/farm/structure');
}

export async function updateFarmProductionUnit(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx || !isManager(ctx.profile.role)) return;
  ...
  await addHistory(ctx, 'farm_production_unit', unitId, 'updated', {
    status: String(formData.get('status') ?? 'active')
  });
  revalidatePath('/farm/structure');
}
...
export async function createWorker(formData: FormData): Promise<void> {
  ...
  if (!data?.id) return;
  await addHistory(ctx, 'workforce_profile', data.id, 'created', {
    full_name: fullName
  });
...
export async function clockWorker(formData: FormData): Promise<void> {
  ...
  if (mode === 'in') {
    const { data } = await ctx.supabase
      ...
      .single();
    if (data?.id)
      await addHistory(ctx, 'workforce_time_entry', data.id, 'clock_in', {
        workforce_profile_id: workerId
      });
  } else {
    ...
    await ctx.supabase
      ...
      .eq('id', openEntry.id);
    await addHistory(ctx, 'workforce_time_entry', openEntry.id, 'clock_out', {
      workforce_profile_id: workerId
    });
  }
  ...
  await ctx.supabase
    ...
    .update({
      ...
      break_minutes: Number(
        String(formData.get('breakMinutes') ?? '').trim() || '0'
      )
    })
    .eq('id', entryId)
    .eq(FARM_ACCOUNT_COLUMN, ctx.profile.farmAccountId);
...
export async function createFarmTask(formData: FormData): Promise<void> {
  ...
  await addHistory(ctx, 'farm_task', data.id, 'created', {
    title,
    assignees: profileIds.length
  });
...
export async function createFarmWorkRequest(formData: FormData): Promise<void> {
  ...
  const requestType =
    String(formData.get('requestType') ?? '').trim() || 'general';
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      raised_by_profile_id:
        toNullable(formData.get('raisedByProfileId')) ?? ctx.profile.id,
      site_id: toNullable(formData.get('siteId')),
      area_id: toNullable(formData.get('areaId')),
      short_description: shortDescription,
      attachments: attachments ? [attachments] : [],
      approval_required:
        String(formData.get('approvalRequired') ?? 'true') !== 'false',
      approval_requested_at:
        String(formData.get('approvalRequested') ?? 'false') === 'true'
  ...
  if (!data?.id) return;
  await addHistory(ctx, 'farm_work_request', data.id, 'created', {
    request_number: requestNumber,
    request_type: requestType
  });
...
export async function transitionFarmWorkRequest(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  ).trim() as (typeof FARM_WORK_REQUEST_STATUSES)[number];
  ...
  const currentStatus =
    existing.status as (typeof FARM_WORK_REQUEST_STATUSES)[number];
  if (
    currentStatus !== nextStatus &&
    !REQUEST_TRANSITIONS[currentStatus]?.has(nextStatus)
  )
    return;
  if (
    (nextStatus === 'approved' ||
  ...
  await ctx.supabase
    ...
    .update({
      status: nextStatus,
      triaged_by_profile_id:
        nextStatus === 'triaged' ? ctx.profile.id : undefined,
      triaged_at:
        nextStatus === 'triaged' ? new Date().toISOString() : undefined,
      approval_requested_at:
        nextStatus === 'waiting_approval'
          ? new Date().toISOString()
          : undefined,
      approved_by_profile_id:
        nextStatus === 'approved' ? ctx.profile.id : undefined,
      approved_at:
        nextStatus === 'approved' ? new Date().toISOString() : undefined,
      approval_notes: toNullable(formData.get('approvalNotes')),
      rejected_by_profile_id:
        nextStatus === 'rejected' ? ctx.profile.id : undefined,
      rejected_at:
        nextStatus === 'rejected' ? new Date().toISOString() : undefined,
      rejection_reason:
        nextStatus === 'rejected'
          ? toNullable(formData.get('rejectionReason'))
          : undefined
    })
    .eq('id', requestId)
    .eq(FARM_ACCOUNT_COLUMN, ctx.profile.farmAccountId);

  await addHistory(ctx, 'farm_work_request', requestId, 'status_changed', {
    from: currentStatus,
    to: nextStatus
  });
  revalidatePath('/farm/requests');
...
export async function convertRequestToWorkOrder(
  ...
  const { data: request } = await ctx.supabase
    .from('farm_work_requests')
    .select(
      'id,request_number,request_type,status,raised_by_profile_id,site_id,area_id,short_description,converted_work_order_id'
    )
    .eq('id', requestId)
    .eq(FARM_ACCOUNT_COLUMN, ctx.profile.farmAccountId)
    .maybeSingle();
  ...
  if (defaultTaskTitle) {
    const { data: task } = await ctx.supabase
      ...
      .single();
    if (task?.id) {
      await addHistory(ctx, 'farm_task', task.id, 'created_from_work_order', {
        farm_work_order_id: createdWorkOrder.id
      });
    }
  }
  ...
  await addHistory(
    ...
    { farm_work_request_id: request.id }
  );
  revalidatePath('/farm/requests');
  revalidatePath('/farm/work-orders');
}

export async function transitionFarmWorkOrder(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  const nextStatus = String(
    formData.get('nextStatus') ?? ''
  ).trim() as (typeof FARM_WORK_ORDER_STATUSES)[number];
  ...
  if (!existing?.status) return;
  const currentStatus =
    existing.status as (typeof FARM_WORK_ORDER_STATUSES)[number];
  if (
    currentStatus !== nextStatus &&
    !WORK_ORDER_TRANSITIONS[currentStatus]?.has(nextStatus)
  )
    return;
  ...
  await addHistory(ctx, 'farm_work_order', workOrderId, 'status_changed', {
    from: currentStatus,
    to: nextStatus
  });
  revalidatePath('/farm/work-orders');
...
export async function createFarmWorkOrderTask(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  if (!task?.id) return;
  await addHistory(ctx, 'farm_task', task.id, 'created_from_work_order', {
    farm_work_order_id: workOrderId
  });
  await addHistory(ctx, 'farm_work_order', workOrderId, 'child_task_created', {
...
export async function transitionFarmTask(formData: FormData): Promise<void> {
  ...
  await ctx.supabase
    ...
    .update({
      status: nextStatus,
      verified_by: nextStatus === 'verified' ? ctx.profile.id : null,
      verified_at: nextStatus === 'verified' ? new Date().toISOString() : null
    })
    .eq('id', taskId);

  await addHistory(ctx, 'farm_task', taskId, 'status_changed', {
    from: task.status,
    to: nextStatus
  });
  revalidatePath('/farm/tasks');
...
export async function addFarmTaskUpdate(formData: FormData): Promise<void> {
  ...
  await ctx.supabase
    ...
    .insert({ task_id: taskId, message, created_by: ctx.profile.id });
...
export async function addTaskProof(formData: FormData): Promise<void> {
  ...
  await addHistory(ctx, 'farm_task', taskId, 'proof_added', {
    proof_type: String(formData.get('proofType') ?? 'text')
  });
  revalidatePath('/farm/tasks');
}
...
export async function reportFarmIncident(formData: FormData): Promise<void> {
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      reported_by: ctx.profile.id,
      escalation_required:
        String(formData.get('escalationRequired') ?? '') === 'on',
...
export async function updateIncidentWorkflow(
  ...
  if (
    ...
    !INCIDENT_TRANSITIONS[currentStatus].has(targetStatus)
  )
    return;
  ...
  await addHistory(ctx, 'farm_incident', incidentId, 'updated', {
    from: currentStatus,
    to: targetStatus
  });
  revalidatePath('/farm/incidents');
...
async function syncIncidentImpacts(
  ...
  const rows = [
    ...
    ...animalIds.map((id) => ({
      workshop_account_id: ctx.profile.farmAccountId,
      incident_id: incidentId,
      impacted_entity_type: 'animal',
      livestock_log_id: id
    })),
    ...assetIds.map((id) => ({
      workshop_account_id: ctx.profile.farmAccountId,
      incident_id: incidentId,
      impacted_entity_type: 'asset',
      asset_id: id
    }))
  ];
...
async function syncFarmVisitPermittedAreas(
  ctx: NonNullable<Awaited<ReturnType<typeof getFarmContext>>>,
  visitId: string,
  areaIds: string[]
) {
...
export async function createFarmVisit(formData: FormData): Promise<void> {
  ...
  if (!visitorName || !purpose || !FARM_VISITOR_TYPE_SET.has(visitorType))
    return;

  const permittedAreaIds = toIdList(formData.getAll('permittedAreaIds'));
  const checkInNow = String(formData.get('checkInNow') ?? '') === 'on';
  const checkedInAt =
    toNullable(formData.get('checkedInAt')) ??
    (checkInNow ? new Date().toISOString() : null);
  ...
  const { data: visit } = await ctx.supabase
    ...
    .insert({
      ...
      biosecurity_complete:
        String(formData.get('biosecurityComplete') ?? '') === 'on',
  ...
  await addHistory(ctx, 'farm_visit', visit.id, 'created', {
    visitor_name: visitorName,
    visitor_type: visitorType,
    checked_in_at: checkedInAt
  });
...
export async function updateFarmVisitDetails(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  await addHistory(ctx, 'farm_visit', visitId, 'updated', {
    source: 'details'
  });
  revalidatePath('/farm/visitors');
}
...
export async function gateCheckVisit(formData: FormData): Promise<void> {
  ...
  if (mode === 'in') {
    await ctx.supabase
      ...
      .eq('id', visitId)
      .eq(FARM_ACCOUNT_COLUMN, ctx.profile.farmAccountId);
    await addHistory(ctx, 'farm_visit', visitId, 'checked_in', {
      checked_in_at: visit.checked_in_at ?? nowIso
    });
  } else {
    ...
    await ctx.supabase
      ...
      .eq('id', visitId)
      .eq(FARM_ACCOUNT_COLUMN, ctx.profile.farmAccountId);
    await addHistory(ctx, 'farm_visit', visitId, 'checked_out', {
...
export async function logFarmExpense(formData: FormData): Promise<void> {
  ...
  if (!data?.id) return;
  await addHistory(ctx, 'expense_log', data.id, 'created', {
    amount_cents: amountCents
  });
...
export async function updateExpenseWorkflow(formData: FormData): Promise<void> {
  ...
  await addHistory(ctx, 'expense_log', expenseId, 'updated', {
    status: String(formData.get('status') ?? 'submitted')
  });
  revalidatePath('/farm/expenses');
}
...
export async function createCropField(formData: FormData): Promise<void> {
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      area_id: toNullable(formData.get('areaId')),
      hectares:
        Number(String(formData.get('hectares') ?? '').trim() || '0') || null,
...
export async function createCropActivityTemplate(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  if (!data?.id) return;
  await addHistory(ctx, 'crop_activity_template', data.id, 'created', {
    activity_type: activityType,
    template_name: templateName
  });
  revalidatePath('/farm/crops');
...
export async function instantiateCropActivityTemplate(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  const plannedDate =
    toNullable(formData.get('plannedDate')) ?? template.planned_date;
  const status = String(formData.get('status') ?? 'planned').trim();
  if (!CROP_ACTIVITY_STATUS_SET.has(status)) return;
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      operator_profile_id:
        toNullable(formData.get('operatorProfileId')) ??
        template.operator_profile_id,
      team_name: toNullable(formData.get('teamName')) ?? template.team_name,
      equipment_used:
        toNullable(formData.get('equipmentUsed')) ?? template.equipment_used,
      input_product:
        toNullable(formData.get('inputProduct')) ?? template.input_product,
      input_rate:
        toNullableNumber(formData.get('inputRate')) ?? template.input_rate,
      ...
      observed_issues:
        toNullable(formData.get('observedIssues')) ?? template.observed_issues,
      status,
      created_by: ctx.profile.id
    })
    .select('id')
...
export async function transitionCropActivity(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  const allowed =
    CROP_ACTIVITY_TRANSITIONS[
      activity.status as (typeof CROP_ACTIVITY_STATUSES)[number]
    ];
  if (!allowed?.has(nextStatus as (typeof CROP_ACTIVITY_STATUSES)[number]))
    return;
  ...
  await ctx.supabase
    ...
    .eq('id', activityId)
    .eq(FARM_ACCOUNT_COLUMN, ctx.profile.farmAccountId);
  await addHistory(ctx, 'crop_activity', activityId, 'status_changed', {
    from: activity.status,
    to: nextStatus,
    sign_off: shouldSignOff && isManager(ctx.profile.role)
  });
  revalidatePath('/farm/crops');
}
...
export async function createLivestockHerd(formData: FormData): Promise<void> {
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      active_count: Number(
        String(formData.get('activeCount') ?? '').trim() || '0'
      )
    })
    .select('id')
    .single();
  ...
  if (!data?.id) return;
  await addHistory(ctx, 'livestock_herd', data.id, 'created', {
    name,
    species
  });
...
export async function createLivestockEvent(formData: FormData): Promise<void> {
  ...
  if (
    !herdId ||
    !eventDate ||
    !LIVESTOCK_EVENT_TYPE_SET.has(eventType) ||
    !LIVESTOCK_EVENT_SCOPE_SET.has(scope)
  )
    return;
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      area_id: toNullable(formData.get('areaId')),
      event_reference:
        toNullable(formData.get('eventReference')) ?? `LE-${Date.now()}`,
      ...
      quantity:
        Number(String(formData.get('quantity') ?? '').trim() || '0') || null,
  ...
  if (followUpDueDate) {
    await ctx.supabase.from('farm_tasks').insert({
      ...
      site_id: toNullable(formData.get('siteId')),
      area_id:
        eventType === 'move' ? toAreaId : toNullable(formData.get('areaId')),
      created_by: ctx.profile.id
    });
  }
...
async function createUnresolvedStockRequestReminder(
  ctx: NonNullable<Awaited<ReturnType<typeof getFarmContext>>>,
  stockRequestId: string,
  requestNumber: string,
  needByAt: string | null
) {
  const dueAt = needByAt
    ? new Date(needByAt).toISOString()
  ...
  await addHistory(
    ...
    stockRequestId,
    'reminder_created',
    { due_at: dueAt, reason: 'unresolved_stock_request' }
  );
}

async function createExpiringDocumentReminder(
  ctx: NonNullable<Awaited<ReturnType<typeof getFarmContext>>>,
  documentId: string,
  ...
  await addHistory(ctx, 'farm_document', documentId, 'reminder_created', {
    due_at: dueAt,
    reason: 'expiring_document'
  });
}

export async function createFarmStockRequest(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  const requestedForType = String(
    formData.get('requestedForType') ?? 'site'
  ).trim();
  ...
  const { data: request } = await ctx.supabase
    ...
    .insert({
      ...
      requester_profile_id:
        toNullable(formData.get('requesterProfileId')) ?? ctx.profile.id,
      ...
      need_by_at: needByAt,
      status:
        String(formData.get('sendForApproval') ?? 'false') === 'true'
          ? 'pending_approval'
          : 'new'
    })
    .select('id')
  ...
  await addHistory(ctx, 'farm_stock_request', request.id, 'created', {
    request_number: requestNumber,
    request_type: requestType,
    need_by_at: needByAt
  });
  await createUnresolvedStockRequestReminder(
    ctx,
    request.id,
    requestNumber,
    needByAt
...
export async function addFarmStockRequestItem(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  await addHistory(ctx, 'farm_stock_request', stockRequestId, 'item_added', {
    item_name: itemName,
    requested_qty: qty,
    unit
  });
...
export async function transitionFarmStockRequest(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  const nextStatus = String(
    formData.get('nextStatus') ?? ''
  ).trim() as (typeof FARM_STOCK_REQUEST_STATUSES)[number];
  ...
  const currentStatus =
    existing.status as (typeof FARM_STOCK_REQUEST_STATUSES)[number];
  if (
    currentStatus !== nextStatus &&
    !STOCK_REQUEST_TRANSITIONS[currentStatus]?.has(nextStatus)
  )
    return;
  if (
  ...
  await ctx.supabase
    ...
    .update({
      ...
      issued_by_profile_id:
        nextStatus === 'issued' || nextStatus === 'partially_issued'
          ? ctx.profile.id
          : undefined,
      issued_at:
        nextStatus === 'issued' || nextStatus === 'partially_issued'
  ...
  if (nextStatus === 'partially_issued' || nextStatus === 'issued') {
    const issuedQty = Number(
      String(formData.get('issuedQty') ?? '').trim() || '0'
    );
      ...
      if (item) {
        ...
        await ctx.supabase
          ...
          .update({
            ...
            item_status:
              cumulativeIssued >= (item.requested_qty ?? 0)
                ? 'issued'
                : 'partially_issued'
          })
          .eq('id', itemId)
...
export async function createFarmDocument(formData: FormData): Promise<void> {
  ...
  const { data } = await ctx.supabase
    ...
    .insert({
      ...
      linked_object_type: String(
        formData.get('linkedObjectType') ?? 'other'
      ).trim(),
      ...
      owner_profile_id:
        toNullable(formData.get('ownerProfileId')) ?? ctx.profile.id,
      visibility_scope: String(
        formData.get('visibilityScope') ?? 'farm'
      ).trim(),
  ...
  await addHistory(ctx, 'farm_document', data.id, 'created', {
    document_number: documentNumber,
    document_type: String(formData.get('documentType') ?? 'other').trim(),
    expiry_date: expiryDate
  });
  if (expiryDate) {
    await createExpiringDocumentReminder(
      ctx,
      data.id,
      documentNumber,
      expiryDate
...
export async function transitionFarmDocument(
  formData: FormData
): Promise<void> {
  const ctx = await getFarmContext();
  if (!ctx) return;
  ...
  await addHistory(ctx, 'farm_document', documentId, 'status_changed', {
